...
export default function Carta() {
  ...
  const eliminar = (codigo: any) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios
          .delete(`https://backendtdc.vercel.app/api/products/${codigo}`)
          .then(() => {
  ...
  const filterProducts = (tipo: any) => {
    const filtered = productosList.filter(
      (product) => (product as any).tipo === tipo
    );
    ...
    <>
      <div className="row my-4">
        <div className="text_nav text-center">
          <a className="tittle">Carta</a>
        </div>
      </div>
      <div className="row justify-content-center">
        <button
          className={`${styles.ingresar} col-3 text-center align-items-center p-1 `}
          type="submit"
        >
          <a href="/admin/registroProducto" className={`${styles.text_form}`}>
            Adicionar producto
          </a>
        </button>
      </div>
          ...
          return (
            <>
              <div className={`${style.carta} col-5 mt-2 mx-2`} key={key}>
                <div key={key}></div>
                <div className="col-12 text-center my-3 fs-5">
                  <b>{(val as any).nombre}</b>
                </div>
                <div className={`${styles.img_inicio} col-12 text-center`}>
                  <img
                    className={`${style.img_product} `}
                    src={(val as any).imagen}
                    alt=""
                  />
                </div>
                <br></br>
                ...
                <div className="col-12 text-center fs-5">
                  <b>Precio </b>
                  {formatNumberCOP((val as any).precio)}
                </div>
                <div className="col-12 justify-content-center">
                  <Link
                    href={`/admin/vistaProductos/editarProducto?codigo=${
                      (val as any).codigo
                    }`}
                    className={`${styles.text_form}`}
                  >
                    <button
                      className={`${styles.ingresar} col-5 text-center align-items-center m-3 p-1 `}
                      type="submit"
                    >
                      Editar
                    </button>
                  ...
                  <button
                    className={`${styles.ingresar} col-5 text-center align-items-center p-1 `}
                    onClick={() => {
                      eliminar((val as any).codigo);
                    }}
                    type="submit"
                  >
                    <a className={`${styles.text_form}`}>Dar de baja</a>
                  </button>
                </div>
              </div>
            </>
