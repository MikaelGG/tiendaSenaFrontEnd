...
export default function FacturaFecha() {
  ...
  useEffect(() => {
    if (storedDate) {
      axios
        .get(`https://backendtdc.vercel.app/api/invoice/${storedDate}`)
        .then((response) => {
  ...
  const fetchFacturaProd = async (nroFactura: any) => {
    try {
      const response = await axios.get(
        `https://backendtdc.vercel.app/api/invoiceProd/modal/${nroFactura}`
      );
  ...
  const generarPDF = async () => {
    ...
    const footer = () => {
      doc.setFontSize(10);
      doc.text(
        "Página " + doc.internal.pages.length,
        pageWidth / 2,
        pageHeight - 10,
        { align: "center" }
      );
    ...
    const addInvoice = async (invoice: any) => {
      ...
      doc.text(
        `Consumidor(a): ${invoice.nombreConsumidor} ${invoice.apellidoConsumidor}`,
        10,
        y
      );
      doc.text(
        `Fecha: ${formatDate(new Date(invoice.fecha))}`,
        pageWidth - 10,
        y,
        { align: "right" }
      );
      ...
      try {
        const response = await axios.get(
          `https://backendtdc.vercel.app/api/invoiceProd/modal/${invoice.nro}`
        );
        ...
        doc.text(
          formatNumberCOP(invoice.total),
          10 + colWidths[0] + colWidths[1] + colWidths[2] + 2,
          y + 7
        );
      ...
      >
          ...
          <div className="col-12 text-center my-3 container">
            ...
            {facturasProdList.map((val, key) => (
              <div className="row mx-2 my-3" key={key}>
                <div className="col-3 mx-2 my-1 texto_drop">
                  {(val as any).nombreProducto}
                </div>
                <div className="col-3 mx-2 my-1 texto_drop">
                  {formatNumberCOP((val as any).precioU)}
                </div>
                <div className="col-2 mx-2 my-1 texto_drop">
                  {formatNumber((val as any).cantidad)}
                </div>
                <div className="col-2 mx-3 my-1 texto_drop">
                  {formatNumberCOP((val as any).subTotal)}
                </div>
              </div>
            ))}
          </div>
        ...
        <Modal.Footer>
          <Button
            ...
            className={`${style.bmodal} col-2 align-items-center p-1`}
    ...
    <>
      <div className="row text-center justify-content-center">
        <div className={`${style.generarPDF}`}>
          <button
            onClick={generarPDF}
            className={`${style.edit} w-30 mx-1 text-center align-items-center p-1`}
          >
            Generar PDF
          </button>
        </div>
          ...
          {facturaList.map((val, key) => (
            <React.Fragment key={key}>
              <div className="row mx-2 my-3">
                <div className="col-3 mx-2 my-1 texto_drop">
                  {formatDate((val as any).fecha)}
                </div>
                <div className="col-3 mx-2 my-1 texto_drop">
                  {(val as any).nombreConsumidor}{" "}
                  {(val as any).apellidoConsumidor}
                </div>
                <div className="col-2 mx-2 my-1 texto_drop">
                  {formatNumberCOP((val as any).total)}
                </div>
                <div className="col-3 mx-2 my-1 texto_drop d-flex flex-row">
                  <a
                    className={`${style.edit} w-50 mx-5 text-center align-items-center p-1`}
                    style={{ textDecoration: "none" }}
                    onClick={() => fetchFacturaProd((val as any).nro)}
                  >
                    Ver
                  </a>
                </div>
